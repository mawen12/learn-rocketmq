...
public class TopicQueueMappingUtils {
	...
	public static Map.Entry<Long, Integer> findMaxEpochAndQueueName(List<TopicQueueMappingDetail> mappingDetailList) {
		long epoch = -1;
		int queueNum = 0;
	...
	public static List<TopicQueueMappingDetail> getMappingDetailFromConfig(Collection<TopicConfigAndQueueMapping> configs) {
		return configs.stream().map(TopicConfigAndQueueMapping::getMappingDetail).filter(Objects::nonNull).collect(Collectors.toList());
	}
	...
	public static Map.Entry<Long, Integer> checkNameEpochNumConsistence(String topic, Map<String, TopicConfigAndQueueMapping> brokerConfigMap) {
		...
		for (Map.Entry<String, TopicConfigAndQueueMapping> entry : brokerConfigMap.entrySet()) {
			...
			if (!broker.equals(mappingDetail.getBname())) {
				throw new RuntimeException(String.format("The broker name is not equals %s != %s", broker, mappingDetail.getBname()));
			}
			...
			if (maxEpoch != -1 && maxEpoch != mappingDetail.getEpoch()) {
				throw new RuntimeException(String.format("epoch does not match %d != %d in %s", maxEpoch, mappingDetail.getEpoch(), broker));
			}
			else {
			...
			if (maxNum != -1 && maxNum != mappingDetail.getTotalQueues()) {
				throw new RuntimeException(String.format("total queue number does not match %d != %d in %s", maxNum, mappingDetail.getTotalQueues(), broker));
			}
			else {
	...
	public static void checkLogicQueueMappingItemOffset(List<LogicQueueMappingItem> items) {
		...
		for (int i = items.size() - 1; i >= 0; i--) {
			...
			if (item.getStartOffset() < 0 || item.getGen() < 0 || item.getQueueId() < 0) {
				throw new RuntimeException("The field is illegal, should not be negative");
			}
			if (items.size() >= 2 && i <= items.size() - 2 && items.get(i).getLogicOffset() < 0) {
			...
			if (item.getEndOffset() != -1 && item.getEndOffset() < item.getStartOffset()) {
	...
	public static Map<Integer, TopicQueueMappingOne> checkAndBuildMappingItems(List<TopicQueueMappingDetail> mappingDetails, boolean replace, boolean checkConsistence) {
		mappingDetails.sort((o1, o2) -> (int) (o2.getEpoch() - o1.getEpoch()));

		int maxNum = 0;
		Map<Integer, TopicQueueMappingOne> globalIdMap = new HashMap<>();
		for (TopicQueueMappingDetail mappingDetail : mappingDetails) {
			if (mappingDetail.getTotalQueues() > maxNum) {
				maxNum = mappingDetail.getTotalQueues();
			}
	...
	public static String writeToTem(TopicRemappingDetailWrapper wrapper, boolean after) {
	...
	public static TopicRemappingDetailWrapper createTopicConfigMapping(String topic, int queueNum, Set<String> targetBrokers, Map<String, TopicConfigAndQueueMapping> brokerConfigMap) {
		...
		Map<Integer, TopicQueueMappingOne> globalIdMap = new HashMap<>();
		Map.Entry<Long, Integer> maxEpochAndNum = new AbstractMap.SimpleEntry<>(System.currentTimeMillis(), queueNum);

		if (!brokerConfigMap.isEmpty()) {
			maxEpochAndNum = checkNameEpochNumConsistence(topic, brokerConfigMap);
			globalIdMap = checkAndBuildMappingItems(new ArrayList<>(getMappingDetailFromConfig(brokerConfigMap.values())), false, true);
			checkIfReusePhysicalQueue(globalIdMap.values());
		...
		Map<Integer, String> oldIdToBroker = new HashMap<>();
		...
		MappingAllocator allocator = buildMappingAllocator(oldIdToBroker, brokerNumMap, null);
		...
		for (Map.Entry<Integer, String> entry : newIdToBroker.entrySet()) {
		...
		checkNameEpochNumConsistence(topic, brokerConfigMap);
		globalIdMap = checkAndBuildMappingItems(getMappingDetailFromConfig(brokerConfigMap.values()), false, true);
		checkIfReusePhysicalQueue(globalIdMap.values());
		checkPhysicalQueueConsistence(brokerConfigMap);
	...
	public static TopicRemappingDetailWrapper remappingStaticTopic(String topic, Map<String, TopicConfigAndQueueMapping> brokerConfigMap, Set<String> targetBrokers) {
		Map.Entry<Long, Integer> maxEpochAndNum = checkNameEpochNumConsistence(topic, brokerConfigMap);
		Map<Integer, TopicQueueMappingOne> globalIdMap = checkAndBuildMappingItems(getMappingDetailFromConfig(brokerConfigMap.values()), false, true);
		checkPhysicalQueueConsistence(brokerConfigMap);
		checkIfReusePhysicalQueue(globalIdMap.values());
		...
		MappingAllocator allocator = buildMappingAllocator(new HashMap<>(), brokerNumMap, brokerNumMapBeforeRemapping);
		...
		for (Map.Entry<Integer, String> mapEntry : expectedIdToBroker.entrySet()) {
			...
			TopicQueueMappingOne mappingOne = globalIdMap.get(queueId);
			assert mappingOne != null;
			if (mappingOne.getBname().equals(broker)) {
				continue;
			}

			final String mapInBroker = broker;
			final String mapOutBroker = mappingOne.getBname();
			...
			List<LogicQueueMappingItem> items = new ArrayList<>(mappingOne.getItems());
		...
		checkPhysicalQueueConsistence(brokerConfigMap);
		checkIfReusePhysicalQueue(globalIdMap.values());
		checkLeaderInTargetBrokers(globalIdMap.values(), targetBrokers);
	...
	public static LogicQueueMappingItem findLogicQueueMappingItem(List<LogicQueueMappingItem> mappingItems, long logicOffset, boolean ignoreNegative) {
		...
		for (int i = mappingItems.size(); i >= 0; i--) {
	...
	public static class MappingAllocator {
		...
		private void freshState() {
			...
			if (brokerNumMapBeforeRemapping != null && !brokerNumMapBeforeRemapping.isEmpty()) {
		...
		private String nextBroker() {
			...
			int tempIndex = currentIndex % leastBrokers.size();
			return leastBrokers.remove(tempIndex);
		}
