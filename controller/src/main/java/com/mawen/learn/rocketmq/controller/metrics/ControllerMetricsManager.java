...
import com.mawen.learn.rocketmq.common.metrics.NopLongUpDownCounter;
...
public class ControllerMetricsManager {
	...
	private static final Logger log = LoggerFactory.getLogger(ControllerMetricsManager.class);

	private static volatile ControllerMetricsManager instance;

	private static final Map<String, String> LABEL_MAP = new HashMap<>();

	private static LongUpDownCounter role = new NopLongUpDownCounter();
	...
	private MetricExporter loggingMetricsExporter;
	...
	public static void recordRole(MemberState.Role newRole, MemberState.Role oldRole) {
		role.add(getRoleValue(newRole) - getRoleValue(oldRole), newAttributesBuilder().build());
	}
	...
	private ControllerMetricsManager(ControllerManager controllerManager) {
		...
		this.config = controllerManager.getControllerConfig();
		if (config.getControllerType().equals(ControllerConfig.JRAFT_CONTROLLER)) {
			LABEL_MAP.put(LABEL_ADDRESS, config.getJraftConfig().getjRaftServerId());
			LABEL_MAP.put(LABEL_GROUP, config.getJraftConfig().getjRaftGroupId());
			LABEL_MAP.put(LABEL_PEER_ID, config.getJraftConfig().getjRaftServerId());
		}
		else {
			LABEL_MAP.put(LABEL_ADDRESS, config.getDLedgerAddress());
			LABEL_MAP.put(LABEL_GROUP, config.getControllerDLegerGroup());
			LABEL_MAP.put(LABEL_PEER_ID, config.getControllerDLegerSelfId());
		}
		this.init();
	}
	...
	private void initMetric(Meter meter) {
		...
		dLedgerDiskUsage = meter.gaugeBuilder(GAUGE_DLEDGER_DISK_USGAE)
				...
				.buildWithCallback(measurement -> {
					...
					if (diskUsage == -1) {
						log.error("calculateFileSizeInPath error, path: {}", path);
						return;
					}
		...
		activeBrokerNum = meter.gaugeBuilder(GAUGE_ACTIVE_BROKER_NUM)
				...
				.ofLongs()
				.buildWithCallback(measurement -> {
					Map<String, Map<String, Integer>> activeBrokersNum = controllerManager.getHeartbeatManger().getActiveBrokersNum();
					activeBrokersNum.forEach((cluster, brokerSetAndNum) -> {
						brokerSetAndNum.forEach((brokerSet, num) -> measurement.record(num, newAttributesBuilder()
								.put(LABEL_CLUSTER_NAME, cluster)
	...
	public void init() {
		...
		if (!checkConfig()) {
			log.error("check metric config failed, will not export metrics");
			return;
		}
		...
		if (StringUtils.isNotBlank(labels)) {
			...
			for (String label : labelList) {
				...
				if (pair.length != 2) {
					log.warn("metrics label is not valid: {}", label);
					continue;
				}
		...
		if (type == MetricsExporterType.OLTP_GRPC) {
			...
			OtlpGrpcMetricExporterBuilder metricExporterBuilder = OtlpGrpcMetricExporter.builder()
					...
					.setTimeout(config.getMetricGrpcExporterTimeOutInMillis(), TimeUnit.MILLISECONDS)
					.setAggregationTemporalitySelector(x -> {
						if (config.isMetricsInDelta() && (x == InstrumentType.COUNTER || x == InstrumentType.OBSERVABLE_COUNTER || x == InstrumentType.HISTOGRAM)) {
			...
			if (StringUtils.isNotBlank(headers)) {
				...
				for (String header : headerList) {
					...
					if (pair.length != 2) {
						log.warn("metricsGrpcExporterHeader is not valid: {}", header);
						continue;
					}
			...
			periodicMetricReader = PeriodicMetricReader.builder(metricExporter)
					.setInterval(config.getMetricGrpcExporterIntervalInMillis(), TimeUnit.MILLISECONDS)
					.build();
		...
		if (type == MetricsExporterType.LOG) {
			...
			loggingMetricsExporter = OtlpJsonLoggingMetricExporter.create(config.isMetricsInDelta() ? AggregationTemporality.DELTA : AggregationTemporality.CUMULATIVE);
			...
			periodicMetricReader = PeriodicMetricReader.builder(loggingMetricsExporter)
					.setInterval(config.getMetricLoggingExporterIntervalInMillis(), TimeUnit.MILLISECONDS)
					.build();
			providerBuilder.registerMetricReader(periodicMetricReader);
		}
		...
		controllerMeter = OpenTelemetrySdk.builder()
